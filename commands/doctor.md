...
5. If dormant or invalid, name the missing requirement or invalid variable (`CYCLES_BASE_URL` or at least one supported `CYCLES_DEFAULT_*` subject field) and link the remedy to the plugin README. If the MCP call fails, distinguish configuration/authentication failures from server unavailability when the error permits it.
