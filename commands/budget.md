...
4. If the call fails because configuration is incomplete, tell the user to set `CYCLES_BASE_URL` and at least one supported `CYCLES_DEFAULT_*` subject field (typically `CYCLES_DEFAULT_TENANT`); also mention `CYCLES_API_KEY` when their Cycles deployment requires authentication.
